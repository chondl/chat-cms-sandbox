---
title: Theater
order: 3
summary: A fall play, a spring musical, and a black-box showcase run entirely by students.
icon: 🎭
showDonate: true
---

Theater produces three shows a year. The fall play is straight drama, the spring
musical is the big one, and the black-box showcase in February is written,
directed, and stage-managed by students.

## What parents do

Build crew meets on Saturdays through October. Dana Example coordinates the
volunteer schedule and will happily find you a job that does not involve power
tools if that is your preference.

## What Booster funds pay for

Set materials, script and score licensing, and the rental lighting package for
the spring musical.
